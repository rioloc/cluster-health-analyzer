			...
			for _, firingAlert := range alerts {
				...
				if match {
					...
					if isAlertSilenced(firingAlert, silencedAlertsMap[alertname]) {
						...
						if len(silences) > 0 {
							updatedAlert["silenced_on"] = model.LabelValue(getMinStartOnFromSilences(silences).Format(time.RFC822))
						} else {
					...
					if _, f := updatedAlertsMap[key]; f {
						...
						isSilenced := lastSilenced && silenced
						...
						updatedAlert["silenced"] = model.LabelValue(fmt.Sprintf("%t", isSilenced))
